...
//自定义表名
func (m *S_MaxNo) TableName() string {
	return "s_maxno"
}

func init() {
	orm.RegisterModel(new(S_MaxNo))
}
...
func GetMaxNo(noname, notype string, length int) string {
	...
	if v, _ := getNoValue(noname, notype); v == 0 {
		...
		no := S_MaxNo{NoName: noname, NoType: notype, NoValue: 1, NoLength: length}
		o.Insert(&no)
	} else {
		v++
		...
		_, err := o.Raw("UPDATE s_maxno SET novalue = ?, nolength = ? WHERE noname = ? and notype = ?", v, length, noname, notype).Exec()
		if err != nil {
			beego.Error("错误：操作数据库错误")
		}
	}
...
func getNoValue(noname, notype string) (int, error) {
	o := orm.NewOrm()
	var maxno S_MaxNo
	err := o.QueryTable("s_maxno").Filter("noname", noname).Filter("notype", notype).One(&maxno, "novalue")
